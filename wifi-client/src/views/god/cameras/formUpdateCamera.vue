<template>
  <div class="formCreateTransport">
    ...
    <form class="params-form"
          @submit.prevent="updateCameraRun">
...
export default {
  ...
  methods: {
    ...
    async updateCameraRun() {
      if (this.numberTC === "") {
        this.numberTC = this.updateTransport.numberTC;
      }
      if (this.numberPTC === "") {
        this.numberPTC = this.updateTransport.numberPTC;
      }
      if (this.VNTO === "") {
        this.VNTO = this.updateTransport.VNTO;
      }
      ...
      let formData = {
        transportId: localStorage.getItem('currentTransport'),
      ...
      let request = await this.GOD_BUILDS_UPDATE(formData);
  ...
  async mounted() {
    ...
    select.selectedIndex = this.updateTransport.type;
  },
  async created() {
    await console.log(this.props.updateCamera);
  }
}
</script>
